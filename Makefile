...
# dev-all 把 dev-deps-check → migrate → API + Worker 串成一条命令，给新人 /
# 端到端冒烟用。脚本 bash 3.2 兼容（macOS 自带），Ctrl-C 优雅停两个子进程。
# 不自动起 docker：caller 自己 `make dev-up` 或起本机依赖，dev-all 只编排。
.PHONY: dev-all
dev-all: SHELL := /usr/bin/env bash
dev-all: ## 起 deps 后并发跑 API + Worker（迁移先行；Ctrl-C 优雅停两个进程）
	@./scripts/dev-all.sh
...
.PHONY: stop-api
stop-api: ## 显式终止占用 API_PORT 的进程（先 TERM，超时后 KILL）
	@pids=$$(lsof -ti:$(API_PORT) 2>/dev/null); \
	if [ -z "$$pids" ]; then \
		echo "port $(API_PORT) is free, nothing to stop."; \
	else \
		echo "sending TERM to pids on port $(API_PORT): $$pids"; \
		kill -TERM $$pids; \
		for i in 1 2 3 4 5; do \
			sleep 1; \
			if ! lsof -ti:$(API_PORT) >/dev/null 2>&1; then \
				echo "port $(API_PORT) stopped."; \
				exit 0; \
			fi; \
		done; \
		pids=$$(lsof -ti:$(API_PORT) 2>/dev/null); \
		if [ -n "$$pids" ]; then \
			echo "TERM timed out; sending KILL to pids on port $(API_PORT): $$pids"; \
			kill -9 $$pids; \
		fi; \
	fi
