...
- [ ] `APP_ENV=production`。这会启用启动期安全 guard：下面两项（`JWT_SECRET`、`AUTH_DEV_TOKEN_ENABLED`）若不合规进程会 **fail-fast 拒绝启动**，无限流（`RATE_LIMIT_PER_MINUTE=0`）会打 warn。把它当成"清单的自动执行器"，但不替代你逐项核对。
- [ ] `JWT_SECRET` 替换成高熵随机值（≥ 32 字节，例：`openssl rand -base64 48`）。`APP_ENV=production` 下占位值 / 空 / 过短会被拦。
- [ ] `AUTH_DEV_TOKEN_ENABLED=false`（路由仍然注册，会返回 `SERVICE_DISABLED`）。`APP_ENV=production` 下设 true 会被拦。
